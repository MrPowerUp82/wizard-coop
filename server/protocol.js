...
export function encodeState(s, viewerId) {
  ...
  return {
    ...
    z: (s.zones || []).filter(near).map(zone => [zone.id, round(zone.x), round(zone.y), round(zone.radius), zone.color]),
    ev: s.events || []
  };
}
...
export function decodeState(c) {
  ...
  return {
    ...
    zones: c.z.map(([id, x, y, radius, color]) => ({ id, x, y, radius, color, ttl: 1 })),
    events: c.ev
  };
}
