...
pads.clear(); pads.set(1, { port: 1, buttons: 0, lx: 128, ly: 128 });
tick(); app.controllers.autoAssign(); app.startRun(); tick();
assert.deepEqual(app.controllers.bindings, [1, null]);
const players = app.game.players;
const p1x = players.me.x;
pads.get(1).lx = 255; tick(30);
assert.ok(players.me.x > p1x);
assert.ok(clips.some(c => c.join() === '0,0,960,544'));
assert.ok(!clips.some(c => c.join() === '0,0,480,544'), 'no split screen allowed on PS Vita');
const p1 = pads.get(1); pads.delete(1); tick(); assert.equal(app.paused, true);
pads.set(1, p1); tick(); assert.equal(app.game.players.me, players.me);
writeFileSync(join(build, 'native-solo2.png'), output.toBuffer('image/png'));
