...
import { createSwitchHud } from './ui/hud.js';
...
// This file only replaces what the browser provided: the DOM HUD and menus (canvas instead),
// keyboard/touch input (Switch controllers) and the page lifecycle.

// DEBUG_CONTROLLERS is defined by build.mjs (true only in debug builds); release bundles drop every debug branch.

// + on the first controller exits nx.js apps by default; here it pauses, like Esc on the web.
...
ctx.fillText('Arcana Survivors · carregando…', W / 2, H / 2);
await loadFonts();

const audio = createAudio();
const wallet = createWallet(); // first save-data access: the console asks for a user profile here, not mid-run
const hud = createSwitchHud();
...
let showDebug = DEBUG_CONTROLLERS;
const perf = DEBUG_CONTROLLERS ? createPerf() : null;
...
const menu = createSwitchMenu({
  controllers, wallet, audio, debugControllers: DEBUG_CONTROLLERS,
...
function startRun({ challenge = null, split = false, character = 0, secondCharacter = 1 }) {
  ...
  hud.announce(challenge ? `Desafio diário · ${curses}` : `${split ? 'Tela dividida' : 'Solo'} · ${CAMPAIGNS[game.campaign].name}`, 'gold');
}
...
function togglePause(force) {
  ...
  // A slot without its controller keeps the run paused until it returns.
...
/** End-of-run summary, the same information as the web results modal. */
...
/** Controller presses for whatever overlay is on top of the run this frame. */
function overlayInput(dt) {
  ...
  if (missing >= 0) {
    ...
    // The returning controller rebinds itself (same Gamepad.id); any free controller may also take over the slot.
  ...
  if (paused) {
    for (const { action } of controllers.events) {
      if (action === 'up' || action === 'down') overlayFocus = 1 - overlayFocus;
      else if (action === 'cancel') togglePause(false);
      else if (action === 'confirm') { if (overlayFocus === 0) togglePause(false); else { endGame(); return; } }
    }
    return;
  }
...
function drawPause() {
  veil(ctx, W, H, 0.6);
  roundPanel(ctx, W / 2 - 300, H / 2 - 170, 600, 330, { fill: COLORS.panelSolid, radius: 18 });
  text(ctx, 'Ⅱ', W / 2, H / 2 - 110, { font: '700 30px Cinzel', align: 'center', color: COLORS.gold });
  text(ctx, 'Ritual pausado', W / 2, H / 2 - 70, { font: '700 32px Cinzel', align: 'center' });
  const missing = missingSlot();
  if (missing >= 0) {
    const message = controllers.coop ? `Controle do Jogador ${missing + 1} desconectado` : 'Nenhum controle conectado';
    text(ctx, message, W / 2, H / 2 - 20, { font: '700 20px Inter', align: 'center', color: COLORS.danger });
    text(ctx, 'Reconecte o controle, ou pressione SL + SR / A em outro controle livre.', W / 2, H / 2 + 14,
      { font: '500 16px Inter', align: 'center', color: COLORS.dim, maxWidth: 560 });
    text(ctx, 'Vida, XP, poderes e personagem continuam guardados.', W / 2, H / 2 + 40, { font: '500 15px Inter', align: 'center', color: COLORS.dim });
    return;
  }
  text(ctx, 'Respire. A floresta pode esperar.', W / 2, H / 2 - 36, { font: '500 17px Inter', align: 'center', color: COLORS.dim });
  menuList(ctx, [{ label: 'Continuar batalha' }, { label: 'Voltar ao menu' }], overlayFocus, W / 2 - 220, H / 2 - 4, 440);
}

function drawResults() {
  veil(ctx, W, H, 0.72);
  roundPanel(ctx, W / 2 - 400, 70, 800, 580, { fill: COLORS.panelSolid, radius: 18, stroke: view.victory ? COLORS.gold : COLORS.line });
  text(ctx, results.title, W / 2, 140, { font: '700 38px Cinzel', align: 'center', color: view.victory ? COLORS.gold : COLORS.ink });
  text(ctx, results.text, W / 2, 178, { font: '500 18px Inter', align: 'center', color: COLORS.dim });
  text(ctx, results.stats, W / 2, 218, { font: '700 16px Inter', align: 'center', color: COLORS.mint, maxWidth: 740 });
  const columns = [W / 2 - 330, W / 2 - 80, W / 2 + 30, W / 2 + 150, W / 2 + 290];
  ['Arcanista', 'Nível', 'Abates', 'Dano', 'Resgates'].forEach((label, i) => text(ctx, label, columns[i], 270, { font: '700 14px Inter', color: COLORS.dim }));
  results.rows.forEach((row, r) => row.forEach((value, i) => text(ctx, value, columns[i], 302 + r * 30, { font: '600 17px Inter', maxWidth: i ? 110 : 230 })));
  text(ctx, results.coins, W / 2, 520, { font: '700 17px Inter', align: 'center', color: COLORS.gold });
  if (results.daily) text(ctx, results.daily, W / 2, 552, { font: '600 16px Inter', align: 'center', color: COLORS.mint });
  if (results.age > 1) text(ctx, 'A / B · voltar ao menu', W / 2, 610, { font: '600 17px Inter', align: 'center', color: COLORS.mint });
}

function debugToggle() {
  // Debug builds only: click a stick to show or hide the controller panel.
  const clicks = HidNpadButton.StickL | HidNpadButton.StickR;
...
function playFrame(now, dt) {
  ...
  if (perf && showDebug) perf.draw(ctx, view, controllers, { split });
}
...
function frame(now) {
  ...
  if (DEBUG_CONTROLLERS && (showDebug || menu.screen === 'controllers')) drawControllerDebug(ctx, controllers, W, H);
}
requestAnimationFrame(frame);

if (DEBUG_CONTROLLERS) {
  /** Hook for the headless smoke test (platforms/switch/test/smoke.mjs); not present in release builds. */
  globalThis.__ARCANA_SWITCH__ = {
